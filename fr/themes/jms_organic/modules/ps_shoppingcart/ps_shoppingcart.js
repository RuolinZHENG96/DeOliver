      ...
      function (event) {
        ...
        $.post(refreshURL, requestData).then(function (resp) {
		  ...
		  if(jpb_addtocart == 'ajax_moveimage') {
			...
			var $element = $(callerElement).parent().parent().parent().find('.product-img1');						
			if (!$element.length)
				$element = $('#bigpic');
		  ...
		  } else if(jpb_addtocart == 'ajax_cartsidebar' && !$('.shoppingcart-sidebar').eq(0).hasClass('open')){
		  		$('#cart_block .tab-title').trigger('click');
				$('#cart_block').addClass('shoppingcart-sidebar');
		  }
        }).fail(function (resp) {
