...
<div class="jmsblog-home-widget">
	<div class="block-title">
		<h3>
			<span>{l s='Latest Post' d='Shop.Theme.Global'}</span>
		</h3>
	</div>
	{if $posts|@count gt 0}
	...
	{foreach from=$posts item=post}		
		...
		<div class="blog-item">
			...
			<div class="post-thumb">
				<a href="#"><img src="{$image_baseurl|escape:'html':'UTF-8'}thumb_{$post.image|escape:'html':'UTF-8'}" alt="{$post.title|escape:'htmlall':'UTF-8'}" class="img-responsive" /></a>			 		
			</div>
			{/if}	
			...
			<ul class="post-meta">
				...
				<li class="post-category"><span>{l s='Category' d='Shop.Theme.Global'} :</span> <a href="{jmsblog::getPageLink('jmsblog-category', $catparams)}">{$post.category_name|escape:'html':'UTF-8'}</a></li>
				{/if}					
				<li class="post-created"><span>{l s='Created' d='Shop.Theme.Global'} :</span> {$post.created|escape:'html':'UTF-8'|date_format:"%B %e, %Y"}</li>
				{if $widget_setting.JBW_SHOW_VIEWS}
				<li class="post-views"><span>{l s='Views' d='Shop.Theme.Global'} :</span> {$post.views|escape:'html':'UTF-8'}</li>
				{/if}
				{if $widget_setting.JBW_SHOW_COMMENT}
				<li class="post-comments"><span>{l s='Comments' d='Shop.Theme.Global'} :</span> {$post.comment_count|escape:'html':'UTF-8'}</li>
				{/if}	
			</ul>
	...
	{else}
		{l s='Dont have any items in this section' d='Shop.Theme.Global'} !
	{/if}
</div>
