	...
	{foreach from=$posts item=post}				
		...
		<div class="blog-item">
			...
			{if $post.link_video && ($show_media == '1')}
				<div class="post-thumb view-blog">
					{$post.link_video|escape:'htmlall':'UTF-8'}
				</div>
			{elseif $post.image && ($show_media == '1')}
				<div class="post-thumb view-blog">
					<a href="{jmsblog::getPageLink('jmsblog-post', $params)|escape:'htmlall':'UTF-8'|replace:'&amp;':'&'}" class="preview-image">
						<img src="{$image_url|escape:'html':'UTF-8'}{$post.image|escape:'html':'UTF-8'}" alt="{$post.title|escape:'htmlall':'UTF-8'}" class="img-responsive img-fluid" />
					</a>
					<a href="{jmsblog::getPageLink('jmsblog-post', $params)|escape:'htmlall':'UTF-8'|replace:'&amp;':'&'}" class="quickview">
		 ...
		 <div class="post-info">
			...
			<ul class="post-meta">
					{if $show_nviews == '1'}
						<li class="post-views">
								<i class="fa fa-eye" aria-hidden="true"></i>{$post.views|escape:'html':'UTF-8'} {l s='Views' d='Modules.JmsPagebuilder'}
						</li>
					{/if}
					{if $show_ncomments == '1'}		
						<li class="post-comments">							
								<i class="fa fa-comment-o" aria-hidden="true"></i>{$post.comment_count|escape:'html':'UTF-8'} {l s='Comments' d='Modules.JmsPagebuilder'}
						</li>
					{/if}
				</ul>
				{if $show_introtext == '1'}	
				...
				{/if}
				{if $show_readmore == '1'}	
					<a class="read-more" href="{jmsblog::getPageLink('jmsblog-post', $params)|escape:'htmlall':'UTF-8'|replace:'&amp;':'&'}">{l s='Read more' d='Modules.JmsPagebuilder'}</a>	
				{/if}
			</div>
		</div>		
	{/foreach}	
