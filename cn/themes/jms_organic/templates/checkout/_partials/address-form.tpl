...
{block name='form_buttons'}
  ...
  {else}
    <form>
      <button type="submit" class="continue btn btn-primary button-small pull-xs-right button-small btn-effect" name="confirm-addresses" value="1">
          {l s='Continue' d='Shop.Theme.Actions'}
      </button>
      {if $customer.addresses|count > 0}
        <a class="js-cancel-address cancel-address pull-xs-right btn-default button-small btn-effect" href="?cancelAddress={$type}">{l s='Cancel' d='Shop.Theme.Actions'}</a>
      {/if}
    </form>
  {/if}
{/block}
