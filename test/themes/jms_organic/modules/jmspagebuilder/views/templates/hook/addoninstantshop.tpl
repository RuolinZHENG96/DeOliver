...
{/if}
<div class="jms-tab col-lg-12 col-md-12 col-sm-12 col-xs-12">
<div class="instant-tabs">
	<ul class="nav nav-tabs" role="tablist">
		{if $showall}
		<li class="active"><a class="instant-btn" data-id="0">{l s='All' d='Shop.Theme.Global'}</a></li>					
		{/if}
	...
	<div class="instantshop-filters btn-group pull-right">
		<a class="dropdown-toggle" data-toggle="dropdown">{l s='Filter' d='Shop.Theme.Global'}<i class="fa fa-angle-down"></i></a>
		<div class="dropdown-menu" role="menu">
			<div class="row">
				<div class="filter col-sm-6 col-md-6 col-xs-12">
					<label>{l s='Sort By' d='Shop.Theme.Global'}</label>
					<ul name="instantshop-orderby" id="instantshop-s-orderby">
						<li data-val="date_add" class="{if $order_by == 'date_add'}active{/if}">{l s='Date Add' d='Shop.Theme.Global'}</li>
						<li data-val="date_upd" class="{if $order_by == 'date_upd'}active{/if}">{l s='Date Update' d='Shop.Theme.Global'}</li>
						<li data-val="price" class="{if $order_by == 'price'}active{/if}">{l s='Price' d='Shop.Theme.Global'}</li>
						<li data-val="position" class="{if $order_by == 'position'}active{/if}">{l s='Position' d='Shop.Theme.Global'}</li>
						<li data-val="name" class="{if $order_by == 'name'}active{/if}">{l s='Name' d='Shop.Theme.Global'}</li>
						<li data-val="id_product" class="{if $order_by == 'id_product'}active{/if}">{l s='Product ID' d='Shop.Theme.Global'}</li>
					</ul>
				</div>
				<div class="filter col-sm-6 col-md-6 col-xs-12">
					<label>{l s='Sort Way' d='Shop.Theme.Global'}</label>
					<ul name="instantshop-orderway" id="instantshop-s-orderway">
						<li data-val="DESC" class="{if $order_way == 'DESC'}active{/if}">{l s='Desc' d='Shop.Theme.Global'}</li>
						<li data-val="ASC" class="{if $order_way == 'ASC'}active{/if}">{l s='Asc' d='Shop.Theme.Global'}</li>		
					</ul>
				</div>
			</div>	
		</div>	
...
<button id="btn" class="load-btn btn-default">{l s='Scroll to Load' d='Shop.Theme.Global'}</button>
{else}
<button id="btn" class="load-btn btn-default">{l s='Load More' d='Shop.Theme.Global'}</button>
{/if}
</div>
