...
<div class="jms-addon jms-video-background">
{if $src}    
...
{if $button_text}
<i class="fa fa-play"></i><a href="{if $button_link}{$button_link nofilter}{else}#{/if}"{if $target == 'new window'} target="_blank"{/if}>{$button_text nofilter}</a>
{/if}
</div>
</div>
</div>