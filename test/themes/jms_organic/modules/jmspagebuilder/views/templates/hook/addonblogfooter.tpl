...
<div class="blogfooter">	
	{foreach from=$posts item=post}				
		...
		<div class="blog-item">
				...
				{if $show_time == '1'}						
				<li>
                   <span> {$post.created|escape:'html':'UTF-8'|date_format:"%d"} </span>
                   <span> {$post.created|escape:'html':'UTF-8'|date_format:"%b"} </span>
                </li>
				{/if}		
			...
			{/if}
			<div class="post-info">
				<a class="post-title" href="{jmsblog::getPageLink('jmsblog-post', $params)|escape:'htmlall':'UTF-8'|replace:'&amp;':'&'}">{$post.title|escape:'htmlall':'UTF-8'}</a>
				{if $show_category == '1'}
				...
				<ul class="post-meta">
					{if $show_nviews == '1'}
						<li class="post-views">{$post.views|escape:'html':'UTF-8'} {l s='Views' d='Modules.JmsPagebuilder'} - 
						</li>
					{/if}
						{if $show_ncomments == '1'}		
				...
				{/if}
				{if $show_readmore == '1'}	
					<a class="readmore" href="{jmsblog::getPageLink('jmsblog-post', $params)|escape:'htmlall':'UTF-8'|replace:'&amp;':'&'}">{l s='[Read more]' d='Modules.JmsPagebuilder'}</a>	
				{/if}
			</div>
		</div>	
	{/foreach}	
